...
const AUDIT_CONTENT = `# Vygo AEO Location and Tool Audit
...
export async function GET() {
  try {
    ...
    const candidates = [
      path.join(cwd, "../../docs/aeo-location-audit.md"),
      path.join(cwd, "docs/aeo-location-audit.md"),
      path.join(cwd, "apps/web/public/docs/aeo-location-audit.md"),
      path.join(cwd, "public/docs/aeo-location-audit.md"),
    ];
    ...
    return new NextResponse(AUDIT_CONTENT, {
      headers: {
        "Content-Type": "text/markdown; charset=utf-8",
      },
    });
