...
- **DNS:** `www.vygo.ai` is a public CNAME to Vercel (`*.vercel-dns-017.com`); apex `vygo.ai` resolves to Vercel A records and 308-redirects to www. Nameservers remain at the domain registrar (`ns77/ns78.domaincontrol.com`).
...
- Invalid input and malformed JSON return HTTP 4xx JSON with no `id` and insert nothing.
