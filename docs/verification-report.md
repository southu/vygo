...
**Date (UTC):** 2026-07-12  
**Re-verified (UTC):** 2026-07-12T05:00Z (iteration 1 of this mission run)  
...
`actions/setup-node` + `.nvmrc`. Verification used Node 24 so readiness
reports `ready=true` (Node 25 is unsupported by `engines` and is not the
verified path).
