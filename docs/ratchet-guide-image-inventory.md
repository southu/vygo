...
equals the count of **captured** rows (4); the 2 **flagged** rows render
frame-only.
...
- **fig-05-mission-control** — **flagged** `/composer`, no image. Reason: the guide's "Start run" button does not exist on the live composer (missions commit via Enqueue / Save & Launch and start via the queue runner; Pause/Resume live on `/queue`).
...
- **Captured** slots (`fig-01`–`fig-04`) render a real `<img>` slot pointing at a
  fresh 1440×900 screenshot under
  `apps/web/public/content/ratchet-guide-assets/`. Guide prose, captions, and
  ordering are unchanged; only each captured figure's image was refreshed.
- **Flagged** slots (`fig-05`, `fig-06`) render the frame-only placeholder and
  have **no** served PNG at their planned asset URL (it returns 404). The
  figcaption still describes what the screen would show; the capture log records
  the reason.
