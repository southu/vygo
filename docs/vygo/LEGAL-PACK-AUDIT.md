# VYGO Legal Pack Publication Record

## Published Documents

| Path                | Role                                                 | Effective date |
| ------------------- | ---------------------------------------------------- | -------------- |
| `terms-of-use.md`   | Terms of Use with acceptable-use requirements        | July 13, 2026  |
| `privacy-policy.md` | Privacy Policy with cookie and analytics disclosures | July 13, 2026  |

## Publication Consistency

- VYGO LLC, a Michigan limited liability company, is the operator identified throughout the legal pack.
- The minimum user age is 18 in both policies.
- Each policy links to the other and uses consistent definitions for the Site.
- The Terms select Michigan law and Michigan state or federal courts, subject to non-waivable consumer rights.
- The Privacy Policy states the data categories, sources, purposes, legal bases, service providers, retention rules, safeguards, transfer approach, and rights process applicable to the Site.
- Questions, privacy requests, or legal notices may be sent to `hello@vygo.ai`. Notices are effective when received.

## Pack Scope

The Terms contain the Site's acceptable-use rules. The Privacy Policy contains its cookie and analytics disclosures. No separate acceptable-use policy, cookie policy, data processing addendum, accessibility policy, or refund policy applies to the current Site. Client services begin only under a separately executed agreement.

Intentional remaining owner-only blanks: none.
